	...
	$.fn.init_addon_totals = function() {
		...
		$(this).on( 'woocommerce-product-addons-update', function() {
			...
			if ( total > 0 && qty > 0 ) {
				...
				html = '<div class="[ product-addon-totals ][ text-center ]"><p>' + woocommerce_addons_params.i18n_addon_total + '<strong><span class="amount">' + formatted_addon_total + '</span></strong></p>';

				if ( formatted_grand_total && '1' == $totals.data( 'show-grand-total' ) ) {
					html = html + '<p>' + woocommerce_addons_params.i18n_grand_total + '<strong><span class="amount">' + formatted_grand_total + '</span></strong></p>';
				}
